...
### 安全证明强化方案：基于游戏的形式化定义与游戏跳跃证明
  ...
  ##### 3.1 安全实验 $\mathsf{Exp}_{\Pi,\mathcal{A}}^{\mathsf{VQ\text{-}Sound}}(\lambda)$（绑定健全性——条件性质）
  ...
  6. **前提条件**：A 提供的证明地址属于令牌绑定集合，即
  ∀(j, id) ∈ R(Q*): ProofAddr_{j,id} ⊆ A_{j,id}^{(t,ν)}
  7. A 胜利当且仅当（在前提条件成立时）：
  acc = 1 ∧ ∃(j, id) ∈ R(Q*): v̂_{j,id} ≠ v_{j,id}^{(t,ν)}

  **注**：该实验为条件性质——仅在地址归属正确的前提下度量对手优势。地址归属正确性本身由 $\mathsf{Exp}^{\mathsf{AB\text{-}Sound}}$（第三章）独立保证。两个实验的组合由 $\mathsf{Exp}^{\mathsf{CV}}$（组合可验证性）统一处理。
  ...
  **Game $G_2$ 中 $\mathcal{A}$ 的胜利概率**：在 $G_2$ 中，锚点未被伪造（$G_1$ 保证），路径未被伪造（$G_2$ 保证），因此 Client 验证得到的每个位值 $b$ 均等于 $B^{(t)}[a]$。又因实验前提条件保证 $\mathcal{A}$ 提供的证明地址 $\mathrm{ProofAddr}_{j,\mathsf{id}} \subseteq \mathcal{A}_{j,\mathsf{id}}^{(t,\nu)}$，Client
  据此重算判定位 $v'_{j,\mathsf{id}} = \bigwedge_{a \in \mathrm{ProofAddr}_{j,\mathsf{id}}} B^{(t)}[a] = \bigwedge_{a \in \mathcal{A}_{j,\mathsf{id}}^{(t,\nu)}} B^{(t)}[a] = v_{j,\mathsf{id}}^{(t,\nu)}$，与真实判定一致。因此 $p_2 = 0$。
  ...
  **最终优势界**：
  ...
  ##### 3.3 新鲜性实验 $\mathsf{Exp}^{\mathsf{Fresh}}$

  **定义 3（新鲜性）**：考虑安全实验 $\mathsf{Exp}_{\Pi,\mathcal{A}}^{\mathsf{Fresh}}(\lambda)$：
  1. Challenger $\mathcal{C}$ 运行 $\mathsf{Setup}(1^\lambda)$，维护签名密钥 $\mathsf{sk}_G$ 与已签名锚点集合 $S_{\mathsf{anchor}}$
  2. $\mathcal{A}$ 可自适应发起更新与令牌生成查询。设当前最新版本为 $t^\star$，对应锚点为 $(t^\star, R_X^{(t^\star)}, \sigma_{t^\star})$
  3. $\mathcal{A}$ 输出目标查询 $Q^\star$ 与响应 $\rho^\star$（含锚点 $(t, R, \sigma_t)$）
  4. $\mathcal{A}$ 胜利当且仅当：$\mathsf{VQ\text{-}Verify}(\rho^\star)=1$ 且 $(t\|R) \notin S_{\mathsf{anchor}}$ 或 $t < t^\star$

  新鲜性优势：$\mathsf{Adv}_{\Pi,\mathcal{A}}^{\mathsf{Fresh}}(\lambda) = \Pr[\mathsf{Exp}_{\Pi,\mathcal{A}}^{\mathsf{Fresh}}(\lambda)=1]$

  **命题 3 证明**：$\mathsf{Exp}^{\mathsf{Fresh}}$ 的胜利事件分解为两个互斥情形，均可归约到 $\mathsf{Exp}^{\mathsf{VQ\text{-}Sound}}$ 中事件 $E_1$（签名伪造）：
  - 情形 1（历史版本重放）：$t' < t^\star$，客户端版本不回退检查直接拒绝
  - 情形 2（混合版本伪造）：$(t\|R') \notin S_{\mathsf{anchor}}$，恰为 $E_1$ 的一个实例，概率 $\leq \mathsf{Adv}_{\mathsf{Sig},\mathcal{B}_1}^{\mathsf{EUF\text{-}CMA}}(\lambda)$

  最终：$\mathsf{Adv}_{\Pi,\mathcal{A}}^{\mathsf{Fresh}}(\lambda) \leq \mathsf{Adv}_{\mathsf{Sig},\mathcal{B}_1}^{\mathsf{EUF\text{-}CMA}}(\lambda)$
  ...
  6. A 胜利当且仅当：
  acc = 1 ∧ ∃(j, id) ∈ R(Q*):
    ProofAddr_{j,id} ⊄ A_{j,id}^{(t,ν)}
  该条件包含两种情形：
    (i) Open_{j,id} 中存在某地址 a ∉ F_{w_j,id}（开封材料含错误地址，
        导致由 {β_i} 索引得到的证明地址偏离真实值）
    (ii) Open_{j,id} 中 ℓ 个地址全部正确，但 QTree 证明中使用的 k 个
         地址不是 ℓ 个承诺地址中由 {β_i} 索引的子集
  ...
  **最终优势界**：
  ...
  实验 Exp^{CV}_{Π,A}(λ):
  1. Challenger C 运行 Setup(1^λ) → (K, st, EDB)，初始化 QTree，
  生成签名密钥对 (sk_G, pk_G)，维护：
    - 已签名锚点集合 S_anchor = ∅
    - 明文承诺表 T_cm[(w,id)] = Cm_{w,id}
    - 完整标签表 T_xtag[(w,id)] = (xtag_1,...,xtag_ℓ)
  C 将 EDB 发送给 A
  2. A 可自适应发起以下预言机查询：
    - O_Update(w, id, op)：C 按扩展更新流程执行，计算 ℓ 个交叉标签
      与承诺 Cm，以认证加密生成 val'，更新 QTree，生成新锚点
      (t, R_X^{(t)}, σ_t)，将 (t||R_X^{(t)}) 加入 S_anchor，
      记录 T_cm 与 T_xtag，将更新消息发送给 A
    - O_GenToken(Q, C_i)：C 按协议执行 GenToken，采样 {β_i}，
      返回令牌 τ 给 A
    - O_Anchor：C 返回当前最新锚点 (t, R_X^{(t)}, σ_t)
  3. A 输出目标查询 Q* 与响应 ρ'*
  4. C 运行 Verify'(K, st, τ*, ρ'*) → (acc, Res')
     Verify' 包含四层验证：版本签名校验、承诺开封验证、
     子集归属与 QTree 路径验证、语义校验
  5. A 胜利当且仅当：
     acc = 1 ∧ ∃(j, id) ∈ R(Q*): v̂_{j,id} ≠ v_{j,id}^{(t,ν)}
 ...
 #### 三、对比总表                                                                                                                                                               
  ...
  | 通信开销 | $O(|\mathsf{Res}|\lambda)$ | $O(|\mathsf{Res}|\lambda)$ | $O(|\mathsf{Res}|\log N)$ | 不可部署 | $O(|\mathsf{Res}|\lambda)$ | $O(|\mathsf{Res}|\lambda)$ | $O(|\mathsf{Cand}|(n{-}1)(k\log M + \ell|\mathbb{G}|))$ |
  | 计算开销（Client） | $O(|\mathsf{Res}|)$ | $O(|\mathsf{Res}|\lambda)$ | $O(|\mathsf{Res}|\log N)$ | 不可部署 | $O(|\mathsf{Res}|)$ | $O(|\mathsf{Res}|)$ | $O(|\mathsf{Cand}|(n{-}1)(k\log M + \ell))$ |
  | 泄露函数 | 标准 SSE 泄露 | 标准 SSE 泄露 | 标准 SSE 泄露 + MHT 根 | 最小泄露 | 标准 SSE 泄露 + 令牌可链接性 | 前向安全泄露 | $\mathcal{L}_{\mathsf{Search}} \cup \mathcal{L}_{\mathsf{Proof}} \cup \mathcal{L}_{\mathsf{Open}}$（Merkle 路径 + 开封标签，不超出 Server 已知范围） |
