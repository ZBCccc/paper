...
**⚠️ 位置盲化缺口（2026-02-25 标注）**：

上述设计假设”客户端可据公开派生规则重建全部地址集合”，但在 Nomos 中，由于 OPRF 盲化，Client 不持有 $K_X$，无法自行计算物理地址。Server 可利用此盲化性进行”地址替换攻击”——用不属于 id 的地址提供合法 Merkle Proof。

该缺口由第三章”嵌入式承诺地址绑定方案”解决：GK 在 Update 阶段将地址集合的承诺嵌入 TSet Payload，验证时 Server 提供开封，Client 核对地址归属。详见 `嵌入式承诺地址绑定方案.md`。
